...
gg <- pandemic_declared(gg, df = italy)
print(gg)
```
...
```{r}
tx <- state %>% dplyr::filter(stateName == "Texas")

gg <- ggplot(tx, aes(x = Date, y = confirmed, label = str_wrap(label, width = 100))) +
  geom_point(color = "#e54e4d") +
  geom_line() +
  labs(y = "Confirmed cases",
       x = "Date",
       caption = paste("Source:", world_data_source),
       title = "Total # of tests in TX over time") + 
  ylim(0, NA)

gg <- text_format(gg, hjust = 1, nudge_x = 20, box_padding = 1)
gg <- pandemic_declared(gg, df = tx, text1 = "A", text2 = "B", text3 = "C")
print(gg)
```
...
```{r}
ca <- state %>% dplyr::filter(stateName == "California")

gg <- ggplot(ca, aes(x = Date, y = confirmed, label = str_wrap(label, width = 100))) +
  geom_point(color = "#e54e4d") +
  geom_line() +
  labs(y = "Confirmed cases",
       x = "Date",
       caption = paste("Source:", world_data_source),
       title = "Total # of tests in California over time")

gg <- text_format(gg, hjust = 1, nudge_x = 20, box_padding = 1)
gg <- pandemic_declared(gg, df = ca)
print(gg)
```
...
```{r}
ny <- state %>% dplyr::filter(stateName == "New York")

gg <- ggplot(ny, aes(x = Date, y = confirmed, label = str_wrap(label, width = 100))) +
  geom_point(color = "#e54e4d") +
  geom_line() +
  labs(y = "Confirmed cases",
       x = "Date",
       caption = paste("Source:", world_data_source),
       title = "Total # of tests in New York over time")

gg <- text_format(gg, hjust = 1, nudge_x = 20, box_padding = .5)
gg <- pandemic_declared(gg, df = ny, text1 = "A", text2 = "B", text3 = "C")
print(gg)
```
